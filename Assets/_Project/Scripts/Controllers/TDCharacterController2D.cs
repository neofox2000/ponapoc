...
{
    public enum DirectionSystem { Scale, Rotation };
    public DirectionSystem directionSystem = DirectionSystem.Rotation;
    ...
    public void changeDirection(int newDirection)
    {
        if (newDirection != 0)
        {
            ...
            {
                case DirectionSystem.Scale:
                    Common.flipObject(_transform, false);
                    break;

                case DirectionSystem.Rotation:
                    Common.flipObject(_transform, true);
                    break;
            }
        }
    }
	...
	private Vector2 GetValidDirectionAdjustment(Vector2 targetPosition, Vector2 direction){
		...
		if(amountOfHits == 0 || (amountOfHits == 1 && selfCollisions && hits2D[0].fraction < 0.5f)){
			// We hit nothing, or we only hit ourselves
			return validPositionAdjustment;
		} else if (amountOfHits == 1 || !selfCollisions || (amountOfHits > 1 && hits2D[0].fraction > 0.5f)){
